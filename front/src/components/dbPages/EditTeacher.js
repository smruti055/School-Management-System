...
const EditTeacher = () => {
    ...
    const loadDetails = async () => {
        try {
            const user = await axios.get(`http://localhost:8080/teacher/edit/${id}`);
            setDT(user.data);
        } catch (error) {
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`http://localhost:8080/teacher/edit/${id}`, dt)
    ...
    <section className="bg-gray-900">
        <div className="px-6 mx-auto h-screen pt-6">
            <nav className='flex justify-between'>
                ...
                {/* <a href='/' className=' text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-md py-3 px-6 text-center'>Logout</a> */}
            </nav>
                        ...
                        <form className="space-y-6" action="post">
                            <div className='flex justify-between'>
                                <div className='w-full mr-4'>
                                    <label for="name" className="block mb-2 text-sm font-medium text-white">Teacher's Name</label>
                            ...
                            <div className='flex justify-between'>
                                ...
                                <div className='w-full ml-4'>
                                    <label for="ph" className="block mb-2 text-sm font-medium text-white">Phone number</label>
                            ...
                            <div className='flex justify-between'>
                                ...
                                <a href='/teachers' className="ml-4 w-full text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center hover:bg-primary-700 focus:ring-primary-800">Go back</a>
                            </div>
                        </form>
                    </div>
                </div>
