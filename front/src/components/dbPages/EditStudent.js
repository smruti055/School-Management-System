...
const EditStudent = () => {
    ...
    const loadDetails = async () => {
        try {
            const user = await axios.get(`http://localhost:8080/student/edit/${id}`);
            setDT(user.data);
        } catch (error) {
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`http://localhost:8080/student/edit/${id}`, dt)
                        ...
                        <form className="space-y-6" action="post">
                            ...
                            <div className='flex justify-between'>
                                ...
                                <div className='w-full ml-4'>
                                    ...
                                    <input type="text" name="classs" value={dt.classs} onChange={changeHandler} id="classs" className="border text-sm rounded-lg focus:ring-primary-600 block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" required="" />
                                </div>
                            </div>
                            <div>
                            ...
                            <div className='flex justify-between'>
                                ...
                                <a href='/students' className="ml-4 w-full text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center hover:bg-primary-700 focus:ring-primary-800">Go back</a>
                            </div>
                        </form>
                    </div>
                </div>
